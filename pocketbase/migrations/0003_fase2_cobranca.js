  ...
  (app) => {
    ...
    var mensalidades = new Collection({
      ...
      indexes: [
        ...
        'CREATE INDEX idx_mens_venc ON mensalidades (vencimento)',
      ],
    })
    ...
    var conciliacao = new Collection({
      ...
      indexes: ['CREATE INDEX idx_conc_status ON conciliacao_bancaria (status)'],
    })
    app.save(conciliacao)
  },
  (app) => {
    var n = ['conciliacao_bancaria', 'mensalidades']
    for (var i = 0; i < n.length; i++) {
      try {
        app.delete(app.findCollectionByNameOrId(n[i]))
      } catch (e) {}
    }
  },
)
